...
<body class="bg-white text-slate-900 antialiased">
    <main class="mx-auto max-w-5xl px-6 py-8">
        ...
        <header class="border-b-2 border-slate-900 pb-5 text-center">
            <p class="text-xl font-bold uppercase tracking-wide text-slate-700">Princess Homes Fatima Chapel</p>
            <h1 class="mt-2 text-2xl font-bold text-slate-950">Monthly Collection Report</h1>
            <p class="mt-1 text-base font-semibold text-slate-600">{{ $collectionType === \App\Models\Collection::BALIK_GASA ? 'BALIK GASA - ' : '' }}{{ $monthLabel }}{{ $collectionType && $collectionType !== \App\Models\Collection::BALIK_GASA ? ' - '.$types[$collectionType] : '' }}</p>
        </header>

        @if ($collectionType === \App\Models\Collection::BALIK_GASA)
